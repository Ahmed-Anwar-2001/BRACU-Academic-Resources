...
export default function CreateForum({ forumToEdit, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setStaus(forumId ? 'Updating' : 'Posting');
      ...
      // Add condition for posting anonymously
      if (postAnonymously) {
        postData = {
          ...postData,
          creator: "Anonymous Post", // Set creator to null if posting anonymously
          anonymous: true, // Add the anonymity flag
        };
      }
  
      console.log('postData:', postData); // Log postData to inspect it
  
      if (forumId) {
        await updateForum(forumId, postData, dispatch);
      } else {
        await createForum(dispatch, postData);
      }
  
      console.log('Forum updated or posted successfully');
  ...
  {/* ... other components ... */}
  <FormControlLabel
    control={<Checkbox checked={postAnonymously} onChange={(e) => setPostAnonymously(e.target.checked)} color="primary" />}
